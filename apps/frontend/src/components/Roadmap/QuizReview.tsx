...
export const QuizReview: React.FC<QuizReviewProps> = ({ review, onReturn, returnLabel = 'Return to Journey Map' }) => {
  ...
  return (
    <motion.div
      ...
      className="space-y-6 flex flex-col flex-1 text-left"
    >
      {/* Score Summary HUD */}
      ...
      <div className="pt-2 border-t border-slate-100 flex justify-center">
        <button
          onClick={onReturn}
          className="w-full max-w-xs bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-6 rounded-xl text-xs tracking-wider transition-all active:scale-[0.98] shadow-md shadow-slate-900/10 flex items-center justify-center gap-2 font-heading cursor-pointer"
        >
