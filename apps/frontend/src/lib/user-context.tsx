...
import type { CrewMember } from './workspace-types';
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // If not selected yet, see if we can pre-populate from active logged-in session
        if (!storedId) {
          const loggedInStr = localStorage.getItem('aws_sgb_rec_user');
          if (loggedInStr) {
            try {
              const parsed = JSON.parse(loggedInStr);
              if (parsed?.id) {
                storedId = parsed.id;
              }
            } catch (e) {
        ...
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              ...
              className="pointer-events-auto flex items-start gap-3 p-4 rounded-xl border bg-white shadow-lg backdrop-blur-md"
              style={{
