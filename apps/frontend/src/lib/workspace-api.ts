import type { Task, CrewMember, TaskStatus, Priority, TaskCategory, Comment, Activity, ProgressHistory } from './workspace-types';
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (Array.isArray(json) && (path.startsWith('/tasks') || path.startsWith('/crew') || path.startsWith('/users'))) {
      return {
        data: json,
        pagination: { total: json.length, page: 1, limit: 150 }
      } as any;
    }
...
export const workspaceApi = {
  ...
  getSecurityLogs: () => request<any[]>('/dashboard/security-logs'),
